...
export class Lobby {
  ...
  constructor(lobbyId: string, host: string, emitter: SocketService) {
    ...
    this.host = host;
    this.players = [];
    this.roomEmitter = (event: string, message: string) =>
      emitter.roomEmit(lobbyId, event, message);
    this.playerEmitter = (player: string, event: string, message: string) =>
      emitter.roomEmit(`${lobbyId}.${player}`, event, message);
    this.addPlayer(host);
  }

  private findPlayer(playerName: string): Player {
    const found: Player = this.players.find(
