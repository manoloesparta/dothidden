...
const [minY, minX] = [32.53626880580044, -117.09652185801386].map(
  (i) => i * factor
);
const [maxY, maxX] = [32.52982856516069, -117.06854105562938].map(
  (i) => i * factor
);
...
export class SocketService {
  ...
  public constructor(server: http.Server) {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("server.lobby.countdown", (data) =>
        handleSocketException(data, lobbyCountdownHandler)
      );
      socket.on("server.player.position", (data) =>
        handleSocketException(data, playerPositionHandler)
      );
      socket.on("server.game.start", (data) =>
        handleSocketException(data, startGameHandler)
      );
...
const playerPositionHandler = (data) => {
  ...
  if (data.player.type === "hider") {
    const player: Hider = lobby.game.hiders.find(
      (hider) => hider.name == data.player.name
    );
    player.moveTo(x, y);
  } else {
  ...
  console.log(
    `Player ${data.player.name} in lobby ${lobby.lobbyId} moved to ${x},${y}`
  );
};
